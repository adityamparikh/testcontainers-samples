...
class ProductControllerTest {
    ...
    static ToxiproxyContainer toxiproxy =
            new ToxiproxyContainer(TOXIPROXY_IMAGE).withNetwork(network).withNetworkAliases(TOXIPROXY_NETWORK_ALIAS);

    @Container
    static GenericContainer<?> wiremockContainer = new GenericContainer<>(WIREMOCK_IMAGE)
            .withNetwork(network)
    ...
    void tearDown() throws IOException {
        Toxic latency = proxy.toxics().getAll().stream()
